...
/datum/computer_file/report/recipient/sec/incident/generate_fields()
	..()
	add_field(/datum/report_field/text_label/header, "NSV Korch Security Department")
...
/datum/computer_file/report/recipient/sec/evidence/generate_fields()
	...
	set_access(access_edit = access_security)
	temp_field = add_field(/datum/report_field/signature, "Wardens's signature")
...
/datum/computer_file/report/recipient/sec/arrest/generate_fields()
	...
	add_field(/datum/report_field/text_label/instruction, "To be filled out by Arresting Officer or Warden. Report must be signed and submitted before the end of the shift!")
	...
	add_field(/datum/report_field/text_label/instruction, "This document MUST be submitted to, and reviwed by, the Head of Security or Warden.")
	add_field(/datum/report_field/signature, "Reporting Officer's signature")
	set_access(access_edit = access_security)
...
/datum/computer_file/report/recipient/sec/restraining/generate_fields()
	..()
	add_field(/datum/report_field/text_label/header, "SEV Torch Security Department")
	add_field(/datum/report_field/text_label/instruction, "To be filled out by the Head of Security, Head of Personnel, or Captain. Report must be signed and submitted for the order to be considered valid. Any paper copies must be stamped.")
...
/datum/computer_file/report/recipient/sec/ltc/generate_fields()
	...
	add_field(/datum/report_field/text_label/instruction, "THIS LICENSE IS ISSUED 'AT-WILL' AND MAY BE REVOKED AT ANY TIME FOR ANY REASON BY THE CAPTAIN, HEAD OF PERSONNEL, OR THE HEAD OF SECURITY. IN THE EVENT OF ILLEGAL CONDUCT, THIS LICENSE MAY BE REVOKED BY ANY LAW ENFORCEMENT OFFICER ACTING IN THE COURSE OF THEIR NORMAL DUTIES. ALL LICENSEES ARE REQUIRED TO ABIDE BY LOCAL LAWS AND REGULATIONS AT ALL TIMES. OPEN CARRY OF LICENSED ITEMS IS GENERALLY NOT PERMITTED UNLESS EXPLICITLY DENOTED. THIS DOCUMENT MUST BE CARRIED BY THE LICENSED PARTY WHEN THEY ARE IN DIRECT OR CONSTRUCTIVE POSSESSION OF THE AFORMENTIONED ITEMS OR WEAPONS THAT THEY ARE AUTHORIZED FOR. COPIES OF THIS DOCUMENT WILL BE FORWARDED TO THE CAPTAIN, HEAD OF PERSONNEL, HEAD OF SECURITY, AND BRIG OFFICER FOR REFERENCE.")
	add_field(/datum/report_field/signature, "Submitting Officer's signature")
	set_access(access_edit = access_hos)
